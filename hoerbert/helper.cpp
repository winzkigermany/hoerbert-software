...
int EjectDriveWin(TCHAR DriveLetter){
    ...
    TCHAR szRootPath[] = L"X:\\";   // "X:\"  -> for GetDriveType
    szRootPath[0] = DriveLetter;

    TCHAR szDevicePath[] = L"X:";   // "X:"   -> for QueryDosDevice
    szDevicePath[0] = DriveLetter;

    TCHAR szVolumeAccessPath[] = L"\\\\.\\X:";   // "\\.\X:"  -> to open the volume
    szVolumeAccessPath[4] = DriveLetter;

    long DeviceNumber = -1;

    // open the storage volume
    HANDLE hVolume = CreateFile(szVolumeAccessPath, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, NULL, NULL);
...
DEVINST GetDrivesDevInstByDeviceNumber(long DeviceNumber, UINT DriveType, TCHAR* szDosDeviceName)
{
    bool IsFloppy = (wcsstr(szDosDeviceName, L"\\Floppy") != nullptr); // who knows a better way?
    ...
    while ( true )	{
        res = SetupDiEnumDeviceInterfaces(hDevInfo, nullptr, guid, dwIndex, &spdid);
        if ( !res ) {
            break;
        }

        dwSize = 0;
        SetupDiGetDeviceInterfaceDetail(hDevInfo, &spdid, nullptr, 0, &dwSize, nullptr); // check the buffer size
        ...
        if ( dwSize!=0 && dwSize<=sizeof(Buf) ) {
            ...
            if ( res ) {
                ...
                */
                ...
                HANDLE hDrive = CreateFile(pspdidd->DevicePath, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, 0, nullptr);
